...
print RES "#Chr_Pos\tMapRef\tMapCov\n";
...
foreach (@het_array) {
  ...
  if ($seq=~/[agtc\.\,]/i) {
    #print "$_\t$seq\n";
    $seq =~ s/$ref{$_}/\./gi;
    my ($ref,$prop,$cov) = &countpos($seq,$seq,$alt{$_});
    print RES "$_\t$ref\t$prop\t$cov\n";
  }
}
...
open (RES, ">$res2") || die "Unable to open results file to write to: $!\n";
print RES "#Chr_Pos\tMapRef\tMapCov\n";
